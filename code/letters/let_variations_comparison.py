...
stim_dir = "../../inputs/datasets/letters" 
...
for s in stim_paths:
    ...
    stim = stim_info.split('_')
    let = f'{stim[0]}_{stim[1]}'
    letters_set.add(let)
...
sorted_letters = sorted(letters_list, key=lambda x: (order[x.split('_')[1]], x.split('_')[0]))
...
for i, layer in enumerate(layers_list, start = 1):
    ...
    letters_avg = np.vstack([averages_dict[f"{letter}"][f"stage{i}"] for letter in sorted_letters])
    
    # For each letter(1):
    for j, let1 in enumerate(sorted_letters): 
        
        # Get letter(1) activations 
        let1_variations = activations_dict[f'{let1}'][f'stage{i}']
        
        # For each letter(2) (again):
        for k, let2 in enumerate(sorted_letters): 
    ...
    for l, let in enumerate(sorted_letters):
        for m, let in enumerate(sorted_letters):
    ...
    for j, script in enumerate(scripts):
        plt.text(-8, 26 * j + 13, script, rotation = 90, fontsize = 20, verticalalignment = 'center', font = 'Avenir')
        plt.text(26 * j + 13, 85, script, rotation = 0, fontsize = 20, horizontalalignment = 'center', font = 'Avenir')
    ...
    # Save plot
    savename = f'model-alexnet_test-letters_layer-{i}_plot-rdm.png'
